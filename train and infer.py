...
Y_val = np.zeros((len(ids_tval), im_height, im_width, 1), dtype=np.float32)
print(X_val.shape)
print(Y_val.shape)
for n, id_ in tqdm(enumerate(ids_val), total=len(ids_val)):
    # Load images
    img = load_img("val_256/image/"+id_, 
                    color_mode = "rgb")
    x_img = img_to_array(img)
    x_img = resize(x_img, (im_height, im_width,3), 
                    mode = 'constant', preserve_range = True)    
    # Load masks
    mask1 = img_to_array(load_img("val_256/label/"+id_, 
...
#this has to be measured from the validation data and not the test data
...
#%%
#find the optimal segmentation threshold from the validation data

Y_val_hat = model1.predict(X_val, 
                           batch_size=1,
                          verbose=1)
print(Y_val_hat.shape)
Y_val_hat_t = (Y_val_hat > 0.5).astype(np.uint8)

#reshape the predictions 
Y_val_hat_int = Y_val_hat.reshape(Y_val_hat.shape[0]*Y_val_hat.shape[1]*Y_val_hat.shape[2], 1)
print(Y_val_hat_int.shape)
Y_val_int = Y_val.reshape(Y_val.shape[0]*Y_val.shape[1]*Y_val.shape[2], 1)
print(Y_val_int.shape)
thresholds = np.linspace(0, 1, 200) 
ious = np.array([jaccard_score(Y_val_int, (Y_val_hat_int > threshold)) for threshold in tqdm(thresholds)])
...
#%%
#measure test performance at the optimal segmentation threshold identified from the validation data
...
pred_all1 = (pred_int+pred_h0_int+pred_h1_int+\
                                  pred_w0_int+pred_w1_int+\
                                      pred_r0_int+\
                                          pred_r1_int)/7
#%%
#find optimal threshold for each case from the validation data by repeating the precedure above
#apply that threshold for the test data
...
#%%
#code to save model predictions as binarized masks for a sample model
...
source_l.sort()
threshold_i = 0.07035 # optimal segmentation threshold identified from the validation data, varies per model
threshold_l = 0.5 # ground truth binarization
model1.load_weights("weights/model1.hdf5")
model1.summary()
...
batch_size = 16
n_epochs = 320
n_cycles = n_epochs / 40  # There are 40 epochs per cycle, a model will be saved at the end of each cycle, we store 8 model snapshots for 8 cycles (i.e., 320/40 = 8)
ca = SnapshotEnsemble(n_epochs, n_cycles, 0.01) #initia learning rate is 1 x 10e-2
callbacks_list = [ca]
...
#%%
#perform inference as before using the optimal segmentation threshold identified from the validation data
...
#%%
#Measure performance at the optimal threshold identified from the validation data
